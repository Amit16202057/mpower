...
function createTable(canEdit, datavals) {
    ...
    if (canEdit === true) {
        var editDelete = '<div class="row"><div class="col-md-6" style="margin-left: 21px;"><a target="_self" id="title_edit" href="#kate" class="btn red bind-edit disabled">' + gettext("edit") + '</a>&nbsp;<a href="#"class="btn red btn-danger">' + gettext("Delete") + '</a>&nbsp;</div></div><div class="top-buffer-min">';
        //dataContainer.append(editDelete);
    }
    var notesSection = '<div id="notes" style="display: none;margin: 0 20px;"> \
    <form action="" onsubmit="return addOrEditNote()" method="post" name="notesform" id="notesform"> \
        ...
        <button style="display: none;" onclick="sendSMS();" type="button" id="send_sms" class="btn blue" >' + gettext("Send SMS") + '</button> \
        </div>\
    </form> \
    <div id="notes-section"></div></div><div class="top-buffer">';
    dataContainer.append(notesSection);

    if (_id_string == 'farmer_query_system') {
        $('#send_sms').show();
    }
...
function deleteData(context, data_id, redirect_route) {
    ...
    $.post(deleteAPIUrl, {
        ...
        .error(function () {
            console.log(gettext('BAD REQUEST'));
        })
    $.ajax({
        url: '/delete-instance/',
        type: "POST",
...
function loadData(context, query, canEdit, id_string) {
    //TODO: show loader
    $.getJSON(mongoAPIUrl, {
        'query': query,
        'limit': 1
        ...
        .success(function (data) {
            ...
            updateButtons(instance_id, id_string);
...
function updateButtons(data_id, id_string) {
    //Make Edit Button visible and add link

    var editbutton = $('a.bind-edit');
    editbutton.removeClass('disabled');
    editbutton.attr('href', '/formbuilder/new-submission/' + id_string + '/' + data_id);
...
function sendSMS() {
    var sms_content = $('textarea#note').val();
    var f_mobile = $("#F-F_Mobile td:nth-child(2)").text();
    console.log(sms_content);
    console.log(f_mobile);
    if (sms_content != '' && f_mobile != '') {
        $.ajax({
            url: '/send_sms_farmer_query/',
            type: 'POST',
            data: {'sms_content': sms_content, 'f_mobile': f_mobile},
            dataType: 'json',
            success: function (response) {
                if (response == 200) {
                    window.alert("SMS has been sent to queue");
                } else {
                    window.alert("SMS has not been sent");
                }
            },
            error: function () {
                alert("error");
    ...
    } else if (sms_content == null || sms_content == '') {
        window.alert("You cannot send empty sms");
    } else if (f_mobile == null || f_mobile == '') {
        window.alert("Farmer's mobile number is not provided");
    }
}
