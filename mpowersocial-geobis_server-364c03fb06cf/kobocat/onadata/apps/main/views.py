...
def survey_summary(request): 
	...
	owner_data = []
        ...
        submission_date_query = "SELECT (json->>'_submission_time')::timestamp::date AS day_of_submission " \
                                    "FROM logger_instance WHERE deleted_at IS NULL and xform_id=" + str(
                xform.id) + ""# and (logger_instance.json->>'_submission_time')::timestamp::date between '" + str(
                #week_ago) + "' and '" + str(today) + "'"
        print submission_date_query
        cursor.execute(submission_date_query)
        fetchVal = cursor.fetchall();
...
def send_sms_farmer_query(request):
    sms_content = request.POST.get('sms_content')
    f_mobile = request.POST.get('f_mobile')

    url = "http://202.73.4.104/g4aw/geobis_sms.php?msisdn=88" + f_mobile + "&text=" + sms_content
    r = requests.get(url)
