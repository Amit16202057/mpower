...
@csrf_exempt
#@login_required
def farmer_request(request,src_request):
    #content_user = get_object_or_404(User, username__iexact=str(username))
    sms=''

    mobile = request.GET.get('m', '-1')
    if src_request=='src_request':
        sms = request.GET.get('sms', '-1')
    if mobile=="-1":
        instance_data_json = {}
        instance_data_json['response_code'] = "9"
        instance_data_json['msg'] = "Invalid Mobile No"
        return HttpResponse(json.dumps(instance_data_json))

    qry="INSERT INTO farmer_request(mobile_no, source_request, request_time, status,sms)	VALUES ('" + mobile + "', '" + src_request + "', now(), 'New', '" + sms + "')"
    print qry
    ...
    try:
        ...
        instance_data_json = {}
        instance_data_json['response_code'] = "0"
        instance_data_json['msg'] = "Received Successfully"
        ...
        instance_data_json = {}
        instance_data_json['response_code'] = "9"
        instance_data_json['msg'] = "DB Error"
        # Rollback in case there is any error
