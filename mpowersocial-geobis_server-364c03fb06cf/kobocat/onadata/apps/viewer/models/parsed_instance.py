...
class ParsedInstance(models.Model):
    ...
    def add_note(self, note):
        print('\n\t(parsed_instance): adding new note..')
        note = Note(instance=self.instance, user_id=self.request.user.username, note=note)
        note.save()
    ...
    def get_notes(self):
        print('\n\t(parsed_instance): get_notes..\n')
        notes = []
        note_qs = self.instance.notes.values(
            'id', 'instance', 'user_id', 'note', 'date_created', 'date_modified')
        for note in note_qs:
            note['date_created'] = \
