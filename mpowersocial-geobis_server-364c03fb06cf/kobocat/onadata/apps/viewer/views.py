...
import pymongo as mongoconnection
...
def custom_data_view(request, username, id_string):
    ...
    if xform.id == 411:
        db_test_column_query = "logger_instance.id ID,logger_instance.json->>'basicinfo/mobile' mobile, (select value_label from xform_extracted where xform_id=411 and field_name='basicinfo/user_type' and value_text=logger_instance.json->>'basicinfo/user_type') User_type,logger_instance.json->>'begin_group_63c587ee/F_farmer_name' Farmer_Name,to_char(logger_instance.date_created,'DD-Mon-YY hh:mi') received "
        column_query = db_test_column_query
        ACTIVATE_CUSTOM_VIEW_QUERY = True

    elif xform.id == 412:
        db_test_column_query = "logger_instance.id ID, logger_instance.json->>'basicinfo/mobile' mobile,(select value_label from xform_extracted where xform_id=412 and field_name='basicinfo/user_type' and value_text=logger_instance.json->>'basicinfo/user_type') User_type,logger_instance.json->>'begin_group_63c587ee/F_farmer_name' Farmer_Name,to_char(logger_instance.date_created,'DD-Mon-YY hh:mi') received"
        column_query = db_test_column_query
        ACTIVATE_CUSTOM_VIEW_QUERY = True

    elif xform.id == 414:
        db_test_column_query = "logger_instance.id ID, logger_instance.json->>'basicinfo/mobile' mobile,(select value_label from xform_extracted where xform_id=414 and field_name='basicinfo/user_type' and value_text=logger_instance.json->>'basicinfo/user_type') User_type,logger_instance.json->>'F/F_Name' Farmer_Name,to_char(logger_instance.date_created,'DD-Mon-YY hh:mi') received "
        column_query = db_test_column_query
        ACTIVATE_CUSTOM_VIEW_QUERY = True


    elif xform.id == 416:
        db_test_column_query = "logger_instance.id ID,logger_instance.json->>'basicinfo/mobile' mobile,(select value_label from xform_extracted where xform_id=416 and field_name='basicinfo/user_type' and value_text=logger_instance.json->>'basicinfo/user_type') User_type,logger_instance.json->>'begin_group_c054f422/F_farmer_name' Farmer_Name,to_char(logger_instance.date_created,'DD-Mon-YY hh:mi') received "
        column_query = db_test_column_query
        ACTIVATE_CUSTOM_VIEW_QUERY = True
    ...
    submission_instance_query = "with t as(SELECT DISTINCT ON (logger_instance.id) " + column_query + " FROM logger_instance LEFT JOIN approval_instanceapproval app_inst ON app_inst.instance_id = logger_instance.id WHERE logger_instance.deleted_at IS NULL and xform_id = " + str(xform.id) + str(sub_query_user) + str(sub_query_date_range) + str(sub_query_status)+") select * from t order by DATE(received) DESC"
    print submission_instance_query
    data_list = []
    col_names = []
    cursor = connection.cursor()
    notes_dict = {}
    if id_string == 'farmer_query_system':
        array = list(xform_instances.find({'_xform_id_string':id_string}))
        #print array[0]
        for i in range(len(array)):
            ...
            for keys, values in data_dict.items():
	            ...
	            if len(values)>0:
                        print data_dict
                        print "adding notes true"
                        notes_dict[str(data_dict['_id'])] = True
    ...
    try:
        if ACTIVATE_CUSTOM_VIEW_QUERY:
            ...
            for eachval in fetchVal:
                ...
                if id_string == 'farmer_query_system':
                    if str(eachval[0]) in notes_dict:
                        eachval = eachval + ('Commented',)
                    else:
                        eachval = eachval + ('Not Commented',)
...
def instance(request, username, id_string):
    ...
    prev_next_query = "SELECT * FROM( (SELECT id FROM logger_instance WHERE xform_id = (SELECT id FROM public.logger_xform where id_string = '" + id_string + "' LIMIT 1) AND id < " + instance_id + " ORDER BY id DESC LIMIT 1) UNION (SELECT id FROM logger_instance WHERE xform_id = (SELECT id FROM public.logger_xform where id_string = '" + id_string + "' LIMIT 1) AND id > " + instance_id + " ORDER BY id ASC LIMIT 1)) maybe_three_records ORDER BY id"
    ...
    form_owner_query = "select (select username from auth_user where id = user_id) as owner from logger_xform where id_string = '"+str(id_string)+"'"
